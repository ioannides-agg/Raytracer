...
public:
  ...
  vec &operator/=(auto scalar) { return *this *= 1 / scalar; }
...
public:
  ...
  vec2 operator-() const {
    return {vec<2, T>::components[0] * -1, vec<2, T>::components[1] * -1};
  }
  vec2 operator+(const vec2 &v) {
    return {vec<2, T>::components[0] + v[0], vec<2, T>::components[1] + v[1]};
  }
  vec2 operator-(const vec2 &v) {
    return {vec<2, T>::components[0] - v[0], vec<2, T>::components[1] - v[1]};
  }
  ...
  vec2 operator/(auto t) {
    return {vec<2, T>::components[0] / t, vec<2, T>::components[1] / t};
  }
  vec2 operator*(auto t) {
    return {vec<2, T>::components[0] * t, vec<2, T>::components[1] * t};
  }
...
public:
  ...
  vec3 operator-() const {
    return {vec<3, T>::components[0] * -1, vec<3, T>::components[1] * -1,
            vec<3, T>::components[2] * -1};
  }
  vec3 operator+(const vec3 &v) const {
    return {vec<3, T>::components[0] + v[0], vec<3, T>::components[1] + v[1],
            vec<3, T>::components[2] + v[2]};
  }
  vec3 operator-(const vec3 &v) const {
    return {vec<3, T>::components[0] - v[0], vec<3, T>::components[1] - v[1],
            vec<3, T>::components[2] - v[2]};
  ...
  vec3 operator/(auto t) const {
    return {vec<3, T>::components[0] / t, vec<3, T>::components[1] / t,
            vec<3, T>::components[2] / t};
  }
  vec3 operator*(auto t) const {
    return {vec<3, T>::components[0] * t, vec<3, T>::components[1] * t,
            vec<3, T>::components[2] * t};
...
template <typename T> class vec4 : public vec<4, T> {
public:
  vec4() {
  ...
  vec4 operator-() const {
    return {vec<4, T>::components[0] * -1, vec<4, T>::components[1] * -1,
            vec<4, T>::components[2] * -1, vec<4, T>::components[3] * -1};
  }
  vec4 operator+(const vec4 &v) {
    return {vec<3, T>::components[0] + v[0], vec<4, T>::components[1] + v[1],
            vec<4, T>::components[2] + v[2], vec<4, T>::components[3] + v[3]};
  }
  vec4 operator-(const vec4 &v) {
    return {vec<3, T>::components[0] - v[0], vec<4, T>::components[1] - v[1],
            vec<4, T>::components[2] - v[2], vec<4, T>::components[3] - v[3]};
  ...
  vec4 operator/(auto t) {
    return {vec<4, T>::components[0] / t, vec<4, T>::components[1] / t,
            vec<4, T>::components[2] / t, vec<4, T>::components[2] / t};
  }
  vec4 operator*(auto t) {
    return {vec<4, T>::components[0] * t, vec<4, T>::components[1] * t,
            vec<4, T>::components[2] * t, vec<4, T>::components[2] * t};
...
template <typename T> vec3<T> cross(const vec3<T> &v, const vec3<T> &u) {
  return vec3<T>(v[1] * u[2] - v[2] * u[1], v[2] * u[0] - v[0] * u[2],
                 v[0] * u[1] - v[1] * u[0]);
}
